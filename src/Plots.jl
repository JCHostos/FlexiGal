function plot_field(model, field; title="Resultado FlexiGal", Deformation_Field=nothing, scale=1.0, invert_y=false)
    ...
    def_plot = () -> begin
        ...
        # Si es uno solo, usamos un tamaño de ventana generoso
        ...
        for (idx, (data, label)) in enumerate(plot_list)
            ...
            # Para evitar el efecto "chiquitito", si es 3D y n=1, simplificamos el layout
            if dim_geo == 3 && n == 1
                ...
                # Barra de colores flotante a la derecha para no robar espacio
                Main.Colorbar(fig[1, 2], plt, width = 15, tellheight = false)
            elseif dim_geo == 3
                # Caso múltiple 3D
                ...
                plt = Main.scatter!(ax, points_to_plot[:, 1], points_to_plot[:, 2]; 
                                    color = data, markersize = 5, colormap = :jet)
